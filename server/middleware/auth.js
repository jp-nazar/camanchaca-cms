...
function requireAuth(req, res, next) {
  ...
  try {
    ...
    const user = db.prepare('SELECT id, email, name, role, auth_provider, avatar_url, email_alerts FROM users WHERE id = ?').get(decoded.id);
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    try {
      ...
      req.user = decoded.recovery
        ...
        : db.prepare('SELECT id, email, name, role, auth_provider, avatar_url FROM users WHERE id = ?').get(decoded.id);
      req.jwtWorkspaceId = decoded.current_workspace_id || null;
    } catch (err) {
