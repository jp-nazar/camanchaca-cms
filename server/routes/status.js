...
router.get('/export', (req, res) => {
  ...
  const user = db.prepare('SELECT id, email, name, role, auth_provider, created_at FROM users WHERE id = ?').get(userId);
  ...
  if (req.query.include_files === 'true') {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=camanchaca-export-${dateStr}.zip`);
  ...
  res.setHeader('Content-Disposition', `attachment; filename=camanchaca-export-${new Date().toISOString().split('T')[0]}.json`);
  res.json(exportData);
});

// User data import (JSON or ZIP with files)
const multer = require('multer');
const importUpload = multer({ dest: path.join(os.tmpdir(), 'camanchaca-import'), limits: { fileSize: 2 * 1024 * 1024 * 1024 } }); // 2GB max
...
router.post('/import', importUpload.single('file'), async (req, res) => {
  ...
  if (req.file) {
    ...
    try {
      ...
      const extractDir = path.join(os.tmpdir(), `camanchaca-import-${Date.now()}`);
  ...
  if (!data || !data.format || !data.format.startsWith('camanchaca-export')) {
    return res.status(400).json({ error: 'Invalid export file. Must be a Camanchaca CMS export JSON.' });
  }
