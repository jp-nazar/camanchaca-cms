  ...
  container.innerHTML = `
    <div class="page-header">
      <div>
        ...
        <div class="subtitle">${'Crea y gestiona listas de contenido'}</div>
      </div>
      <div style="display:flex;gap:8px;align-items:center">
        <label style="display:flex;align-items:center;gap:6px;font-size:13px;color:var(--text-secondary);cursor:pointer">
          <input type="checkbox" id="showAutoToggle" ${showAutoGenerated ? 'checked' : ''}>
          ${'Mostrar autogeneradas'}
        </label>
        <button class="btn btn-primary" id="createPlaylistBtn">${'+ Nueva lista'}</button>
      </div>
    </div>
    <div id="playlistGrid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:16px">
      <div style="color:var(--text-muted);padding:40px;text-align:center">${'Cargando...'}</div>
    </div>
  `;
    ...
    if (!playlists.length) {
      grid.innerHTML = `
        <div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--text-muted)">
          ...
          <h3 style="margin-bottom:8px;color:var(--text-primary)">${'Aún no hay listas'}</h3>
          <p>${'Crea tu primera lista para organizar contenido para tus pantallas.'}</p>
        </div>
      `;
      return;
    }
    ...
    if (!filtered.length) {
      grid.innerHTML = `
        <div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--text-muted)">
          ${playlists.length ? 'Todas las listas son autogeneradas. Activa "Mostrar autogeneradas" para verlas.' : ''}
        </div>
      `;
      return;
    }
      ...
      <a href="#/playlists/${esc(p.id)}" class="playlist-card" style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px;text-decoration:none;color:inherit;display:block;transition:border-color 0.15s;cursor:pointer">
        <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:12px">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            ${p.status === 'draft' ? `<span style="font-size:10px;padding:2px 6px;border-radius:4px;background:#78350f;color:#fbbf24">${'borrador'}</span>` : ''}
          </div>
          <div style="font-size:12px;color:var(--text-muted);white-space:nowrap;margin-left:12px">${(p.item_count === 1 ? '1 elemento' : (p.item_count) + ' elementos')}</div>
        </div>
        ...
        <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text-muted)">
          ...
          ${p.display_count ? `<span>${(p.display_count === 1 ? '1 pantalla' : (p.display_count) + ' pantallas')}</span>` : ''}
        </div>
      </a>
    `).join('');
  } catch (err) {
    grid.innerHTML = `<div style="grid-column:1/-1;color:var(--text-muted);padding:40px;text-align:center">${'Error al cargar las listas: ' + (esc(err.message))}</div>`;
  }
}
  ...
  try {
    ...
    container.innerHTML = `
      <div style="padding:40px;text-align:center;color:var(--text-muted)">
        <p>${'Error al cargar las listas: ' + (esc(err.message))}</p>
        <a href="#/playlists" class="btn btn-secondary" style="margin-top:16px">${'Volver a listas'}</a>
      </div>
    `;
  }
}
  ...
  container.innerHTML = `
    ...
    <div id="draftBanner" style="background:#78350f;border:1px solid #92400e;border-radius:var(--radius-lg);padding:14px 20px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;gap:16px">
      <div style="display:flex;align-items:center;gap:10px;color:#fbbf24">
        ...
        <div>
          ...
          <div style="font-size:12px;color:#fcd34d;opacity:0.85">${hasPublished ? 'Los dispositivos siguen mostrando la última versión publicada.' : 'Esta lista nunca se ha publicado. Los dispositivos no mostrarán nada hasta que la publiques.'}</div>
        </div>
      </div>
      <div style="display:flex;gap:8px;flex-shrink:0">
        ${hasPublished ? `<button class="btn btn-secondary btn-sm" id="discardDraftBtn" style="color:#fbbf24;border-color:#92400e">${'Descartar cambios'}</button>` : ''}
        <button class="btn btn-sm" id="publishBtn" style="background:#f59e0b;color:#000;font-weight:600;border:none">${'Publicar'}</button>
      </div>
    </div>
    ` : ''}
    ...
    <div class="page-header">
      <div style="display:flex;align-items:center;gap:12px">
        <a href="#/playlists" style="color:var(--text-muted);text-decoration:none;font-size:20px" title="${'Atrás'}">&larr;</a>
        <div>
          <h1 id="playlistTitle" style="cursor:pointer" title="${'Clic para renombrar'}">${esc(playlist.name)}</h1>
          <div class="subtitle" id="playlistDesc" style="cursor:pointer" title="${'Clic para editar descripción'}">${playlist.description ? esc(playlist.description) : `<span style="opacity:0.5">${'Agregar una descripción...'}</span>`}</div>
          ${playlist.display_count ? `<div style="font-size:12px;color:var(--text-muted);margin-top:4px">${(playlist.display_count === 1 ? 'Asignada a 1 pantalla' : 'Asignada a ' + (playlist.display_count) + ' pantallas')}</div>` : ''}
        </div>
      </div>
      <div style="display:flex;gap:8px">
        <button class="btn btn-primary" id="addItemBtn">${'+ Agregar contenido'}</button>
        <button class="btn btn-secondary" id="deletePlaylistBtn" style="color:var(--danger)">${'Eliminar lista'}</button>
      </div>
    </div>
  ...
  if (!items.length) {
    itemsEl.innerHTML = `
      <div style="text-align:center;padding:40px;color:var(--text-muted);border:2px dashed var(--border);border-radius:var(--radius-lg)">
        <p style="margin-bottom:8px">${'Esta lista está vacía'}</p>
        <p style="font-size:13px">${'Haz clic en "Agregar contenido" para añadir elementos.'}</p>
      </div>
    `;
    return;
  }
  ...
  itemsEl.innerHTML = items.map((item, i) => `
    <div class="playlist-item" data-item-id="${item.id}" data-index="${i}" draggable="true" style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:12px 16px;display:flex;align-items:center;gap:12px;cursor:grab;transition:border-color 0.15s">
      ...
      <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
        <button class="btn-icon item-move" data-item-id="${item.id}" data-dir="up" title="${'Subir'}" aria-label="${'Subir'}" ${i === 0 ? 'disabled' : ''} style="color:var(--text-muted);background:none;border:none;cursor:pointer;padding:4px;border-radius:4px;${i === 0 ? 'opacity:0.3;cursor:not-allowed' : ''}">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="18 15 12 9 6 15"/></svg>
        </button>
        <button class="btn-icon item-move" data-item-id="${item.id}" data-dir="down" title="${'Bajar'}" aria-label="${'Bajar'}" ${i === items.length - 1 ? 'disabled' : ''} style="color:var(--text-muted);background:none;border:none;cursor:pointer;padding:4px;border-radius:4px;${i === items.length - 1 ? 'opacity:0.3;cursor:not-allowed' : ''}">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg>
        </button>
        <button class="btn-icon item-remove" data-item-id="${item.id}" title="${'Eliminar'}" aria-label="${'Quitar elemento'}" style="color:var(--text-muted);background:none;border:none;cursor:pointer;padding:4px;border-radius:4px">
...
function inlineEdit(playlist, field) {
  ...
  if (isName) {
    ...
    async function save() {
      ...
      newEl.title = 'Clic para renombrar';
    ...
    async function save() {
      ...
      newEl.style.cursor = 'pointer';
      newEl.title = 'Clic para editar descripción';
      if (playlist.description) {
        newEl.textContent = playlist.description;
      } else {
        newEl.innerHTML = `<span style="opacity:0.5">${'Agregar una descripción...'}</span>`;
      }
      input.replaceWith(newEl);
