...
async function renderList(container) {
  container.innerHTML = `
    <div class="page-header">
      <div><h1>${'Muros de video'} <span class="help-tip" data-tip="${'Combina varias pantallas en una sola grande. Configura el tamaño de la cuadrícula, arrastra dispositivos a posiciones, ajusta la compensación de bisel. Asigna contenido para reproducir en todos los dispositivos.'}">?</span></h1><div class="subtitle">${'Combina varias pantallas en una sola grande'}</div></div>
  ...
  document.getElementById('newWallBtn').onclick = async () => {
    const name = prompt('Nombre del muro de video:');
    if (!name) return;
  ...
  try {
    ...
    if (!walls.length) {
      grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><h3>${'Aún no hay muros'}</h3><p>${'Crea un muro para combinar varias pantallas.'}</p></div>`;
      return;
    }
    ...
    grid.innerHTML = walls.map(w => `
      <div class="content-item" style="cursor:pointer" onclick="window.location.hash='#/wall/${w.id}'">
        ...
        <div class="content-item-body">
          ...
          <div class="content-item-size">${'Cuadrícula ' + (w.grid_cols) + 'x' + (w.grid_rows) + ' • ' + (w.devices?.length || 0) + ' dispositivos'}</div>
        </div>
      </div>
    `).join('');
...
async function renderWallEditor(container, wallId) {
  ...
  } catch { container.innerHTML = `<div class="empty-state"><h3>${'Muro no encontrado'}</h3></div>`; return; }
    ...
    <div style="display:flex;gap:16px;align-items:flex-start">
      <div style="flex:1;min-width:0">
        ...
        <div style="display:flex;gap:12px;margin-top:12px;align-items:center;flex-wrap:wrap">
          <div class="form-group" style="margin:0"><label style="font-size:11px;color:var(--text-muted)">${'Columnas'}</label><input type="number" id="gridCols" class="input" value="${wall.grid_cols}" min="1" max="20" style="width:70px"></div>
  ...
  function renderSidebar() {
    ...
    sidebar.innerHTML = unassigned.length
      ? unassigned.map(d => `
        ...
        `).join('')
      : `<p style="color:var(--text-muted);font-size:12px;text-align:center;padding:12px">${'Todos los dispositivos asignados'}</p>`;
