...
async function renderList(container) {
  container.innerHTML = `
    <div class="page-header">
      <div><h1>${'Equipos'} <span class="help-tip" data-tip="${'Crea equipos para compartir dispositivos con otros usuarios. Los propietarios gestionan el equipo, los editores pueden cambiar contenido/listas, los espectadores solo monitorean.'}">?</span></h1><div class="subtitle">${'Gestiona equipos y acceso compartido'}</div></div>
  ...
  try {
    ...
    if (!teams.length) {
      list.innerHTML = `<div class="empty-state"><h3>${'Aún no hay equipos'}</h3><p>${'Crea un equipo para compartir dispositivos con otros usuarios.'}</p></div>`;
      return;
    }
          ...
          <div style="padding:20px">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
              ...
              <div>
                ...
                <div style="font-size:12px;color:var(--text-muted)">${'Tu rol: ' + (team.my_role)} &middot; ${(team.member_count === 1 ? '1 miembro' : (team.member_count) + ' miembros')}</div>
              </div>
            </div>
          </div>
        </div>
...
async function renderTeamDetail(container, teamId) {
  ...
  } catch { container.innerHTML = `<div class="empty-state"><h3>${'Equipo no encontrado'}</h3></div>`; return; }
  ...
  container.innerHTML = `
    ...
    <div class="page-header">
      ...
      <div style="display:flex;gap:8px">
        <button class="btn btn-danger btn-sm" id="deleteTeamBtn">${'Eliminar equipo'}</button>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">
      <div class="settings-section" style="margin:0">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <h3 style="font-size:15px">${'Miembros (' + (team.members?.length || 0) + ')'}</h3>
          <button class="btn btn-secondary btn-sm" id="inviteMemberBtn">${'+ Invitar'}</button>
        </div>
        <div id="membersList">
          ${(team.members || []).map(m => `
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">
              ...
              <select class="input" style="max-width:100px;width:100%;background:var(--bg-input);font-size:12px;padding:4px 8px" data-member-id="${m.user_id}" ${m.role === 'owner' ? 'disabled' : ''}>
                ...
                <option value="owner" ${m.role === 'owner' ? 'selected' : ''}>${'Propietario'}</option>
              </select>
              ${m.role !== 'owner' ? `<button class="btn-icon" data-remove-member="${m.user_id}" style="color:var(--danger)" title="${'Eliminar'}">&#10005;</button>` : ''}
            </div>
          `).join('') || `<p style="color:var(--text-muted);font-size:13px">${'Sin miembros'}</p>`}
        </div>
      </div>

      <div class="settings-section" style="margin:0">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <h3 style="font-size:15px">${'Dispositivos compartidos (' + (devices.length) + ')'}</h3>
        ...
        <div id="teamDevicesList">
          ${devices.map(d => `
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">
              ...
              <button class="btn-icon" data-remove-device="${d.id}" style="color:var(--danger)" title="${'Quitar del equipo'}">&#10005;</button>
            </div>
          `).join('') || `<p style="color:var(--text-muted);font-size:13px">${'No hay dispositivos compartidos con este equipo'}</p>`}
        </div>
      </div>
    </div>
  `;
  ...
  document.getElementById('inviteMemberBtn').onclick = async () => {
    const email = prompt('Email para invitar:');
    if (!email) return;
    const role = prompt('Rol (viewer, editor o owner):', 'editor');
    if (!['viewer', 'editor', 'owner'].includes(role)) { showToast('Rol no válido', 'error'); return; }
    try {
  ...
  document.getElementById('addDeviceToTeam').onchange = async (e) => {
    ...
    try {
      ...
      showToast('Dispositivo agregado al equipo', 'success');
      renderTeamDetail(container, teamId);
    } catch (err) { showToast(err.message, 'error'); }
  };
  ...
  container.querySelectorAll('[data-remove-device]').forEach(btn => {
    btn.onclick = async () => {
      try {
        ...
        showToast('Dispositivo quitado del equipo', 'success');
        renderTeamDetail(container, teamId);
      } catch (err) { showToast(err.message, 'error'); }
    };
  });
