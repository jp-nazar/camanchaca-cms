...
export function render(container) {
  ...
  container.innerHTML = `
    <div class="page-header">
      <div><h1>${'Diseñador de contenido'} <span class="help-tip" data-tip="${'Crea señalización personalizada con elementos en vivo: relojes, clima, tickers RSS, cuentas regresivas, códigos QR. Publica como widget o exporta como PNG.'}">?</span></h1><div class="subtitle">${'Crea contenido dinámico para señalización'}</div></div>
      <div style="display:flex;gap:8px">
        <button class="btn btn-secondary" id="loadDesignBtn">${'Cargar diseño'}</button>
        <button class="btn btn-secondary" id="exportPngBtn">${'Exportar PNG'}</button>
        <button class="btn btn-primary" id="publishBtn">${'Publicar en biblioteca'}</button>
      </div>
    </div>
    <div style="display:flex;gap:20px">
      ...
      <div style="flex:1">
        ...
        <p style="font-size:11px;color:var(--text-muted);margin-top:8px">${'Haz clic en los elementos para seleccionar. Arrastra para reposicionar. La vista previa se actualiza en tiempo real.'}</p>
      </div>
      <!-- Sidebar -->
      <div style="width:300px;display:flex;flex-direction:column;gap:12px;max-height:calc(100vh - 120px);overflow-y:auto">
        ...
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:12px">
          <h4 style="font-size:13px;margin-bottom:10px">${'Agregar elemento'}</h4>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
            ...
            <button class="btn btn-secondary btn-sm" id="addImage" style="justify-content:center">&#128247; ${'Imagen'}</button>
            <button class="btn btn-secondary btn-sm" id="addVideo" style="justify-content:center">&#127916; ${'Video'}</button>
            <button class="btn btn-secondary btn-sm" id="addClock" style="justify-content:center">&#128339; ${'Reloj'}</button>
            <button class="btn btn-secondary btn-sm" id="addDate" style="justify-content:center">&#128197; ${'Fecha'}</button>
            ...
            <button class="btn btn-secondary btn-sm" id="addCountdown" style="justify-content:center">&#9201; ${'Cuenta regresiva'}</button>
            <button class="btn btn-secondary btn-sm" id="addWebpage" style="justify-content:center">&#127760; ${'Página web'}</button>
          </div>
        </div>
        <!-- Background -->
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:12px">
          <h4 style="font-size:13px;margin-bottom:8px">${'Fondo'}</h4>
          ...
          <div style="display:flex;gap:6px">
            ...
            <button class="btn btn-secondary btn-sm" id="bgImageBtn">${'Imagen'}</button>
          </div>
        ...
        <div id="propPanel" style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:12px;display:none">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
            <h4 style="font-size:13px">${'Propiedades'}</h4>
            <button class="btn btn-danger btn-sm" id="deleteEl">${'Eliminar'}</button>
          </div>
          <div id="propFields"></div>
        </div>
        <!-- Layers -->
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:12px">
          <h4 style="font-size:13px;margin-bottom:8px">${'Capas'}</h4>
  ...
  document.getElementById('addText').onclick = () => addElement({ type: 'text', x: 10, y: 60, text: 'Tu texto aquí', fontSize: 24, fontFamily: 'Arial', color: '#FFFFFF', bold: false, shadow: false });
  ...
  document.getElementById('addWeather').onclick = () => {
    const location = prompt('Ciudad, Estado:', 'Milwaukee, WI');
  ...
  document.getElementById('addTicker').onclick = () => {
    const url = prompt('URL del feed RSS:', 'https://feeds.bbci.co.uk/news/rss.xml');
  ...
  document.getElementById('addQR').onclick = () => {
    const data = prompt('URL del código QR:', 'https://example.com');
  ...
  document.getElementById('addCountdown').onclick = () => {
    const target = prompt('Fecha objetivo (AAAA-MM-DD):', '2026-04-01');
    if (target) addElement({ type: 'countdown', x: 20, y: 40, fontSize: 48, color: '#FFFFFF', targetDate: target, label: 'Próximamente' });
  };
  ...
  document.getElementById('publishBtn').onclick = async () => {
    try {
      ...
      const res = await fetch('/api/widgets', {
        ...
        body: JSON.stringify({ widget_type: 'text', name: 'Diseño ' + (new Date().toLocaleDateString()), config: { html: generateInnerHTML(), css: '', background: bgValue } })
      });
      if (res.ok) showToast('¡Publicado como widget! Asígnalo a una zona de diseño.', 'success');
      else showToast('Falló la publicación', 'error');
  ...
  document.getElementById('exportPngBtn').onclick = async () => {
    ...
    } catch (err) { showToast('Falló la exportación: ' + (err.message), 'error'); }
  };
  ...
  document.getElementById('loadDesignBtn').onclick = () => {
    ...
    input.onchange = () => {
      ...
      reader.onload = (ev) => {
        ...
        } catch { showToast('Archivo de diseño no válido', 'error'); }
      };
  ...
  elements.forEach((el, i) => {
    ...
    switch (el.type) {
      ...
      case 'shape':
        ...
        break;
      case 'weather':
        html += `<div style="position:absolute;left:${el.x}%;top:${el.y}%;font-size:${el.fontSize / 10}vw;color:${el.color};${border}${cursor}" data-idx="${i}" id="weather_${i}">&#9925; ${'Cargando...'}</div>`;
        break;
      case 'ticker':
        html += `<div style="position:absolute;left:${el.x}%;top:${el.y}%;width:${el.width}%;height:${el.height}%;background:${el.bgColor};overflow:hidden;display:flex;align-items:center;${border}" data-idx="${i}">
          <div style="white-space:nowrap;animation:ticker ${el.speed || 30}s linear infinite;font-size:${el.fontSize / 10}vw;color:${el.color}" id="ticker_${i}">${'Cargando noticias...'}</div>
        </div>`;
        break;
      case 'qr':
    ...
    if (el.type === 'countdown') {
      ...
      if (cdEl && el.targetDate) {
        const update = () => {
          ...
          if (diff <= 0) { cdEl.textContent = '¡AHORA!'; return; }
    ...
    if (el.type === 'ticker') {
      ...
      if (tEl && el.feedUrl) {
        fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(el.feedUrl)}`).then(r => r.json()).then(d => {
          tEl.textContent = (d.items || []).map(item => item.title).join('  •  ') || 'Sin elementos';
        }).catch(() => { tEl.textContent = 'Feed no disponible'; });
      }
    }
  });
}
...
function updateProps() {
  ...
  if (el.type === 'text') {
    html += `<div class="form-group"><label>${'Texto'}</label><input type="text" class="input" value="${el.text}" data-prop="text"></div>
      <div class="form-group"><label>${'Tamaño'}</label><input type="range" min="8" max="120" value="${el.fontSize}" data-prop="fontSize" style="width:100%"><span style="font-size:11px;color:var(--text-muted)">${el.fontSize}px</span></div>
      <div class="form-group"><label>${'Fuente'}</label><select class="input" style="background:var(--bg-input)" data-prop="fontFamily">${FONTS.map(f => `<option ${f === el.fontFamily ? 'selected' : ''}>${f}</option>`).join('')}</select></div>
      ...
      <label style="font-size:12px;display:flex;gap:6px;margin:4px 0"><input type="checkbox" ${el.bold ? 'checked' : ''} data-prop="bold"> ${'Negrita'}</label>
    ...
    html += `<div class="form-group"><label>${'Tamaño'}</label><input type="range" min="16" max="120" value="${el.fontSize}" data-prop="fontSize" style="width:100%"></div>
    ...
    if (el.type === 'video') html += `<label style="font-size:12px;display:flex;gap:6px;margin:8px 0"><input type="checkbox" ${el.muted ? 'checked' : ''} data-prop="muted"> ${'Silenciado'}</label>
      <label style="font-size:12px;display:flex;gap:6px;margin:4px 0"><input type="checkbox" ${el.loop ? 'checked' : ''} data-prop="loop"> ${'Bucle'}</label>`;
  } else if (el.type === 'shape') {
    html += `<div style="display:flex;gap:6px"><div class="form-group" style="flex:1;margin:0"><label>W%</label><input type="number" class="input" value="${Math.round(el.width)}" data-prop="width"></div>
      <div class="form-group" style="flex:1;margin:0"><label>H%</label><input type="number" class="input" value="${Math.round(el.height)}" data-prop="height"></div></div>
      <div class="form-group"><label>${'Color'}</label><input type="color" value="${el.color}" data-prop="color" style="width:100%;height:28px;border:none"></div>
      <div class="form-group"><label>${'Opacidad'}</label><input type="range" min="0" max="1" step="0.1" value="${el.opacity}" data-prop="opacity" style="width:100%"></div>
    ...
    html += `<div class="form-group"><label>${'Ubicación'}</label><input type="text" class="input" value="${el.location}" data-prop="location"></div>
      <div class="form-group"><label>${'Tamaño'}</label><input type="range" min="16" max="80" value="${el.fontSize}" data-prop="fontSize" style="width:100%"></div>
      <div class="form-group"><label>${'Color'}</label><input type="color" value="${el.color}" data-prop="color" style="width:100%;height:28px;border:none"></div>`;
  } else if (el.type === 'ticker') {
    html += `<div class="form-group"><label>${'URL del feed'}</label><input type="text" class="input" value="${el.feedUrl}" data-prop="feedUrl"></div>
      <div class="form-group"><label>${'Velocidad (segundos)'}</label><input type="number" class="input" value="${el.speed}" data-prop="speed"></div>
      <div class="form-group"><label>${'Color del texto'}</label><input type="color" value="${el.color}" data-prop="color" style="width:100%;height:28px;border:none"></div>
      <div class="form-group"><label>${'Color de fondo'}</label><input type="text" class="input" value="${el.bgColor}" data-prop="bgColor"></div>`;
  } else if (el.type === 'countdown') {
    html += `<div class="form-group"><label>${'Fecha objetivo'}</label><input type="date" class="input" value="${el.targetDate}" data-prop="targetDate"></div>
      <div class="form-group"><label>${'Etiqueta'}</label><input type="text" class="input" value="${el.label}" data-prop="label"></div>
      <div class="form-group"><label>${'Tamaño'}</label><input type="range" min="16" max="100" value="${el.fontSize}" data-prop="fontSize" style="width:100%"></div>
      <div class="form-group"><label>${'Color'}</label><input type="color" value="${el.color}" data-prop="color" style="width:100%;height:28px;border:none"></div>`;
  }
  ...
  html += `<button class="btn btn-secondary btn-sm" style="width:100%;margin-top:8px;justify-content:center" onclick="(() => {
    ...
    a.click();
  })()">${'Guardar archivo de diseño'}</button>`;
...
function updateLayers() {
  ...
  `).join('') || `<p style="color:var(--text-muted)">${'Aún no hay elementos'}</p>`;
