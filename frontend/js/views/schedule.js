...
export async function render(container) {
  ...
  container.innerHTML = `
    <div class="page-header">
      <div><h1>${'Horario'} <span class="help-tip" data-tip="${'Calendario semanal visual para programación. Haz clic en Agregar horario para crear franjas. Configura recurrencia para repetir contenido. La prioridad mayor anula la menor. Los horarios de dispositivo anulan los de grupo.'}">?</span></h1><div class="subtitle">${'Calendario de programación de contenido'}</div></div>
    </div>
    <div class="schedule-controls" style="display:flex;gap:12px;margin-bottom:16px;align-items:center;flex-wrap:wrap">
      ...
      <button class="btn btn-secondary btn-sm" id="nextWeek">${'Siguiente >'}</button>
      <button class="btn btn-primary btn-sm" id="addScheduleBtn">${'Agregar horario'}</button>
    </div>
    ...
    <div class="modal-overlay" id="scheduleModal" style="display:none">
      <div class="modal" style="width:480px">
        <div class="modal-header"><h3 id="schedModalTitle">${'Agregar horario'}</h3>
        ...
        <div class="modal-body">
          <div class="form-group"><label>${'Aplicar a'}</label>
            ...
            ${groups.length === 0 ? `<div id="schedNoGroups" style="display:none;color:var(--text-muted);font-size:12px;margin-top:4px">${'Aún no hay grupos creados. Créalos en la página Pantallas.'}</div>` : ''}
            <div id="schedZoneNote" style="display:none;color:var(--text-muted);font-size:11px;margin-top:4px">${'Nota: Los horarios por zona dependen del diseño. Asegúrate de que todos los dispositivos del grupo usan el mismo diseño.'}</div>
          </div>
          <div class="form-group"><label>${'Sobrescribir lista'}</label>
            <select id="schedPlaylist" class="input" style="background:var(--bg-input)">
              <option value="">${'— Sin sobrescribir lista —'}</option>
              ${playlists.map(p => `<option value="${esc(p.id)}">${esc(p.name)}${p.status === 'draft' ? ' ' + '(borrador)' : ''}</option>`).join('')}
            </select>
          </div>
          <div class="form-group"><label>${'Sobrescribir diseño'}</label>
            <select id="schedLayout" class="input" style="background:var(--bg-input)">
              <option value="">${'— Sin sobrescribir diseño —'}</option>
              ${layouts.map(l => `<option value="${esc(l.id)}">${esc(l.name)}</option>`).join('')}
            </select>
          </div>
          <div class="form-group"><label>${'Contenido'} <span style="color:var(--text-muted);font-weight:normal;font-size:11px">${'(elemento único, opcional)'}</span></label>
          ...
          <div class="form-group"><label>${'Título (opcional)'}</label><input type="text" id="schedTitle" class="input" placeholder="${'p. ej., Lista mañana'}"></div>
          <div style="display:flex;gap:12px">
            <div class="form-group" style="flex:1"><label>${'Hora de inicio'}</label><input type="time" id="schedStart" class="input" value="09:00"></div>
            <div class="form-group" style="flex:1"><label>${'Hora de fin'}</label><input type="time" id="schedEnd" class="input" value="17:00"></div>
          </div>
          <div class="form-group"><label>${'Repetir'}</label>
            <select id="schedRepeat" class="input" style="background:var(--bg-input)">
              ...
              <option value="FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR">${'Días laborables'}</option>
              <option value="FREQ=WEEKLY;BYDAY=SA,SU">${'Fines de semana'}</option>
              <option value="FREQ=WEEKLY">${'Semanal'}</option>
            </select>
          </div>
          <div class="form-group"><label>${'Prioridad'}</label><input type="number" id="schedPriority" class="input" value="0" min="0" max="100"></div>
        ...
        <div class="modal-footer" style="display:flex;justify-content:space-between;gap:8px">
          <button class="btn btn-danger" id="deleteScheduleBtn" style="display:none">${'Eliminar'}</button>
          <div style="display:flex;gap:8px;margin-left:auto">
            <button class="btn btn-secondary" onclick="document.getElementById('scheduleModal').style.display='none'">${'Cancelar'}</button>
            <button class="btn btn-primary" id="saveScheduleBtn">${'Guardar'}</button>
          </div>
        </div>
      </div>
    </div>
  ...
  function editSchedule(ev) {
    editingId = ev.id;
    document.getElementById('schedModalTitle').textContent = 'Editar horario';
  ...
  document.getElementById('addScheduleBtn').onclick = () => {
    editingId = null;
    document.getElementById('schedModalTitle').textContent = 'Agregar horario';
