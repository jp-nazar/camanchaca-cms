...
async function renderList(container) {
  container.innerHTML = `
    <div class="page-header">
      <div><h1>${'Diseños'} <span class="help-tip" data-tip="${'Crea diseños de pantalla multi-zona. Usa plantillas o crea uno propio. Arrastra zonas para posicionar, cambia tamaño con la esquina. Asigna diseños a dispositivos desde la pestaña Lista.'}">?</span></h1><div class="subtitle">${'Diseños de pantalla y plantillas'}</div></div>
      <button class="btn btn-primary" id="newLayoutBtn">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
        ${'Nuevo diseño'}
      </button>
    </div>
    <h3 style="margin-bottom:12px;font-size:14px;color:var(--text-secondary)">${'Plantillas'}</h3>
    <div class="content-grid" id="templateGrid"></div>
    <h3 style="margin:24px 0 12px;font-size:14px;color:var(--text-secondary)">${'Mis diseños'}</h3>
  ...
  try {
    ...
    document.getElementById('layoutGrid').innerHTML = custom.length ? custom.map(l => renderLayoutCard(l, false)).join('') :
      `<div class="empty-state" style="grid-column:1/-1"><p>${'Aún no hay diseños personalizados'}</p></div>`;
  ...
  return `
    <div class="content-item" style="cursor:pointer">
      ...
      <div class="content-item-body">
        ...
        <div class="content-item-size">${zonesText}${isTemplate ? ' • ' + 'Plantilla' : ''}</div>
      </div>
      <div class="content-item-actions">
        ${isTemplate
          ? `<button class="btn btn-primary btn-sm" data-use-template="${layout.id}">${'Usar plantilla'}</button>`
          : `<button class="btn btn-secondary btn-sm" data-edit-layout="${layout.id}">${'Editar'}</button>`
        }
        <button class="btn btn-danger btn-sm" data-delete-layout="${layout.id}" data-layout-name="${layout.name}" style="margin-left:4px">${'Eliminar'}</button>
      </div>
    </div>
  `;
}
...
async function renderEditor(container, layoutId) {
  ...
  } catch { container.innerHTML = `<div class="empty-state"><h3>${'Diseño no encontrado'}</h3></div>`; return; }
  ...
  container.innerHTML = `
    ...
    <div class="page-header">
      ...
      <div style="display:flex;gap:8px">
        <button class="btn btn-secondary btn-sm" id="addZoneBtn">${'Agregar zona'}</button>
        <button class="btn btn-primary btn-sm" id="saveLayoutBtn">${'Guardar'}</button>
      </div>
    </div>
    <div style="display:flex;gap:20px">
      ...
      <div style="width:280px">
        ...
        <div id="zoneProperties" style="margin-top:16px;display:none">
          <h3 style="font-size:14px;margin-bottom:12px">${'Propiedades'}</h3>
          <div class="form-group"><label>${'Nombre'}</label><input type="text" id="propName" class="input"></div>
          ...
          <div class="form-group"><label>${'Ancho (%)'}</label><input type="number" id="propW" class="input" min="1" max="100" step="0.1"></div>
  ...
  document.getElementById('addZoneBtn').onclick = () => {
    zones.push({ id: null, name: 'Zona ' + (zones.length + 1), x_percent: 10, y_percent: 10, width_percent: 30, height_percent: 30, z_index: 0, zone_type: 'content', fit_mode: 'cover', background_color: '#000000', sort_order: zones.length });
