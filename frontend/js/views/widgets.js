  ...
  return new Promise(async (resolve) => {
    ...
    overlay.innerHTML = `
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px;width:100%;max-width:640px;max-height:90vh;display:flex;flex-direction:column">
        <h3 style="margin:0 0 12px;color:var(--text-primary)">${title || 'Seleccionar imagen'}</h3>
        <input type="text" id="cpSearch" class="input" placeholder="${'Buscar imágenes...'}" style="margin-bottom:12px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;gap:8px;flex-wrap:wrap">
          ...
          <div style="display:flex;gap:8px;margin-left:auto">
            ...
            ${multiple ? `<button class="btn btn-primary" id="cpDone">${'Listo'}</button>` : ''}
          </div>
        </div>
      </div>`;
    ...
    const updateCount = () => {
      ...
      if (el && multiple) el.textContent = (selected.size) + ' seleccionadas';
    };
    ...
    function renderList() {
      ...
      if (!filtered.length) {
        list.innerHTML = `<div style="color:var(--text-muted);padding:32px;text-align:center;font-size:13px">${items.length ? 'Sin coincidencias.' : 'No hay imágenes en tu biblioteca. Sube imágenes primero desde la Biblioteca de contenido.'}</div>`;
        return;
      }
  ...
  overlay.innerHTML = `
    <div style="width:100%;max-width:1400px;height:90vh;background:var(--bg-card);border-radius:8px;display:flex;flex-direction:column;overflow:hidden;border:1px solid var(--border)">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 16px;border-bottom:1px solid var(--border)">
        <strong style="color:var(--text-primary)">${'Previsualización'}</strong>
        <button class="btn btn-secondary btn-sm" id="pvClose">${'Cerrar'}</button>
      </div>
...
export async function render(container) {
  container.innerHTML = `
    <div class="page-header">
      <div><h1>${'Widgets'} <span class="help-tip" data-tip="${'Elementos de contenido dinámico: relojes en vivo, clima, tickers RSS, texto, páginas web y feeds sociales. Crea un widget y asígnalo a la lista de un dispositivo.'}">?</span></h1><div class="subtitle">${'Agrega contenido dinámico a tus diseños'}</div></div>
    ...
    <div class="modal-overlay" id="widgetModal" style="display:none">
      <div class="modal" style="width:560px">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-secondary" id="previewWidgetBtn">${'Previsualizar'}</button>
          <button class="btn btn-primary" id="saveWidgetBtn">${'Guardar'}</button>
        </div>
      </div>
    </div>
  `;
  ...
  function showConfigForm(type, config) {
    ...
    let html = `<div class="form-group"><label>${'Nombre del widget'}</label><input type="text" id="wName" class="input" value="${escAttr(config._name || typeName)}"></div>`;
    ...
    switch (type) {
      case 'clock':
        html += `
          ...
          <div class="form-group"><label>${'Zona horaria'}</label><input type="text" id="wTimezone" class="input" value="${config.timezone || 'America/Chicago'}" placeholder="America/New_York"></div>
          <div class="form-group"><label>${'Tamaño de fuente (px)'}</label><input type="number" id="wFontSize" class="input" value="${config.font_size || 64}"></div>
          ...
          <div class="form-group"><label>${'Fondo'}</label><input type="color" id="wBg" value="${config.background || '#000000'}" style="width:60px;height:32px;border:none"></div>`;
        break;
      case 'weather':
        html += `
          <div class="form-group"><label>${'Ubicación'}</label><input type="text" id="wLocation" class="input" value="${config.location || ''}" placeholder="${'Ciudad, Estado'}"></div>
          ...
          <div class="form-group"><label>${'Tamaño de fuente'}</label><input type="number" id="wFontSize" class="input" value="${config.font_size || 48}"></div>
          <div class="form-group"><label>${'Color'}</label><input type="color" id="wColor" value="${config.color || '#FFFFFF'}" style="width:60px;height:32px;border:none"></div>`;
        break;
      case 'rss':
        html += `
          <div class="form-group"><label>${'URL del feed'}</label><input type="text" id="wFeedUrl" class="input" value="${config.feed_url || ''}" placeholder="https://example.com/feed.xml"></div>
          <div class="form-group"><label>${'Velocidad de desplazamiento (segundos)'}</label><input type="number" id="wScrollSpeed" class="input" value="${config.scroll_speed || 30}"></div>
          <div class="form-group"><label>${'Máx. elementos'}</label><input type="number" id="wMaxItems" class="input" value="${config.max_items || 10}"></div>
          <div class="form-group"><label>${'Tamaño de fuente'}</label><input type="number" id="wFontSize" class="input" value="${config.font_size || 24}"></div>
          <div class="form-group"><label>${'Color'}</label><input type="color" id="wColor" value="${config.color || '#FFFFFF'}" style="width:60px;height:32px;border:none"></div>
          <div class="form-group"><label>${'Fondo'}</label><input type="color" id="wBg" value="${config.background || '#000000'}" style="width:60px;height:32px;border:none"></div>`;
        break;
      case 'text':
        html += `
          <div class="form-group"><label>${'Contenido HTML'}</label><textarea id="wHtml" class="input" rows="6" style="font-family:monospace;font-size:12px">${config.html || '<h1 style="color:white;text-align:center;margin-top:40px">Hello World</h1>'}</textarea></div>
      ...
      case 'webpage':
        html += `
          ...
          <div class="form-group"><label>${'Intervalo de actualización (segundos, 0 = nunca)'}</label><input type="number" id="wRefresh" class="input" value="${config.refresh_interval || 0}"></div>`;
        break;
      case 'social':
        html += `
          ...
          <div class="form-group"><label>${'Consulta'}</label><input type="text" id="wQuery" class="input" value="${config.query || ''}" placeholder="${'@usuario o #hashtag'}"></div>`;
        break;
      case 'directory-board':
        html += `
          <div class="form-group"><label>${'Título'}</label><input type="text" id="wTitle" class="input" value="${escAttr(config.title)}" placeholder="${'Almacén Lincoln'}"></div>
          <div class="form-group"><label>${'Logotipo (opcional)'}</label><div id="wLogoBox"></div></div>
          <div class="form-group"><label>${'Texto del pie'}</label><input type="text" id="wFooter" class="input" value="${escAttr(config.footer_text)}" placeholder="${'Consultas de arrendamiento: Contacto...'}"></div>
          <div class="form-group">
            <label>${'Imágenes de fondo (opcional)'}</label>
            <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">${'Las imágenes se alternan cada 15 segundos al 30% de opacidad. Agrega varias para rotación.'}</div>
            <div id="wBgList"></div>
            <button type="button" class="btn btn-secondary btn-sm" id="wBgAdd" style="margin-top:8px">${'+ Agregar imagen de fondo'}</button>
          </div>
          <div class="form-group" style="display:flex;gap:12px;flex-wrap:wrap">
            <div style="flex:1;min-width:140px"><label>${'Tema'}</label><select id="wTheme" class="input" style="background:var(--bg-input)">
              <option value="dark" ${!config.theme || config.theme === 'dark' ? 'selected' : ''}>${'Oscuro'}</option>
              <option value="light" ${config.theme === 'light' ? 'selected' : ''}>${'Claro'}</option>
            </select></div>
            <div style="flex:1;min-width:140px"><label>${'Velocidad de desplazamiento'}</label><select id="wSpeed" class="input" style="background:var(--bg-input)">
              <option value="slow" ${config.scroll_speed === 'slow' ? 'selected' : ''}>${'Lenta'}</option>
              <option value="medium" ${!config.scroll_speed || config.scroll_speed === 'medium' ? 'selected' : ''}>${'Media'}</option>
              <option value="fast" ${config.scroll_speed === 'fast' ? 'selected' : ''}>${'Rápida'}</option>
            </select></div>
            <div style="flex:1;min-width:140px"><label>${'Columnas'}</label><select id="wCols" class="input" style="background:var(--bg-input)">
          ...
          <div class="form-group">
            ...
            <button type="button" class="btn btn-secondary btn-sm" id="dbAddCategory" style="margin-top:10px">${'+ Agregar categoría'}</button>
          </div>`;
        break;
    }
  ...
  function renderDirCategories(opts = {}) {
    ...
    if (!dirState.categories.length) {
      cont.innerHTML = `<div style="padding:20px;text-align:center;color:var(--text-muted);border:1px dashed var(--border);border-radius:6px;font-size:13px">${'Agrega tu primer piso o departamento para empezar'}</div>`;
      return;
    }
      ...
      const entryRows = (cat.entries || []).map((e, j) => `
        <div class="db-entry" style="display:flex;gap:6px;align-items:flex-start;margin-bottom:8px;flex-wrap:wrap">
          ...
          <div style="display:flex;flex-direction:column;gap:4px;flex:1;min-width:140px">
            <input type="text" class="input" data-entry-name="${i}-${j}" value="${escAttr(e.name)}" placeholder="${'Nombre del inquilino'}">
            <input type="text" class="input" data-entry-subtitle="${i}-${j}" value="${escAttr(e.subtitle)}" placeholder="${'Detalles (opcional)'}" style="font-size:12px">
          </div>
          <label style="display:flex;align-items:center;gap:4px;font-size:12px;white-space:nowrap;color:var(--text-muted);padding-top:8px">
            <input type="checkbox" data-entry-avail="${i}-${j}" ${e.available ? 'checked' : ''}> ${'Disponible'}
          </label>
          <button type="button" class="btn-icon" data-entry-up="${i}-${j}" ${j === 0 ? 'disabled' : ''} title="${'Subir'}" style="padding:4px 6px">&#8593;</button>
          <button type="button" class="btn-icon" data-entry-down="${i}-${j}" ${j === cat.entries.length - 1 ? 'disabled' : ''} title="${'Bajar'}" style="padding:4px 6px">&#8595;</button>
          <button type="button" class="btn-icon" data-entry-delete="${i}-${j}" title="${'Eliminar entrada'}" style="padding:4px 6px;color:#ff6b6b">&#215;</button>
        </div>
      `).join('');
      ...
      return `
        <div class="db-category" style="border:1px solid var(--border);border-radius:6px;margin-bottom:8px;padding:8px;background:var(--bg-input)">
          <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
            <button type="button" class="btn-icon" data-cat-toggle="${i}" title="${cat._expanded ? 'Contraer' : 'Expandir'}" style="padding:4px 8px">${cat._expanded ? '&#9660;' : '&#9654;'}</button>
            <input type="text" class="input" data-cat-name="${i}" value="${escAttr(cat.name)}" placeholder="${'p. ej. Primer piso'}" style="flex:1;min-width:140px;font-weight:600">
            <span style="font-size:11px;color:var(--text-muted);white-space:nowrap">${entryCount} ${entriesLabel}</span>
            <button type="button" class="btn-icon" data-cat-up="${i}" ${i === 0 ? 'disabled' : ''} title="${'Subir'}" style="padding:4px 6px">&#8593;</button>
            <button type="button" class="btn-icon" data-cat-down="${i}" ${i === dirState.categories.length - 1 ? 'disabled' : ''} title="${'Bajar'}" style="padding:4px 6px">&#8595;</button>
            <button type="button" class="btn-icon" data-cat-delete="${i}" title="${'Eliminar categoría'}" style="padding:4px 6px;color:#ff6b6b">&#215;</button>
          </div>
          ${cat._expanded ? `
            <div style="padding:10px 0 4px 4px;margin-top:8px;border-top:1px solid var(--border)">
              ${entryRows || `<div style="font-size:12px;color:var(--text-muted);padding:4px 0 8px">${'Aún no hay entradas'}</div>`}
              <button type="button" class="btn btn-secondary btn-sm" data-add-entry="${i}" style="margin-top:4px">${'+ Agregar entrada'}</button>
            </div>
          ` : ''}
        </div>
      `;
  ...
  function wireDirHandlers(opts = {}) {
    ...
    cont.querySelectorAll('[data-cat-delete]').forEach(b => b.onclick = () => {
      ...
      const label = dirState.categories[i].name || '(sin nombre)';
      if (!confirm('¿Eliminar la categoría "' + (label) + '" y todas sus entradas?')) return;
    ...
    if (dirState.logo_url) {
      box.innerHTML = `
        <div style="display:flex;align-items:center;gap:10px;padding:10px;border:1px solid var(--border);border-radius:6px;background:var(--bg-input)">
          ...
          <button type="button" class="btn btn-secondary btn-sm" id="wLogoChange">${'Cambiar'}</button>
          <button type="button" class="btn-icon" id="wLogoClear" title="${'Quitar'}" style="color:#ff6b6b;padding:4px 8px">&#215;</button>
        </div>`;
      ...
      box.innerHTML = `<button type="button" class="btn btn-secondary btn-sm" id="wLogoChoose">${'Elegir logotipo'}</button>`;
      document.getElementById('wLogoChoose').onclick = pickLogo;
    }
  }

  async function pickLogo() {
    const url = await openContentPicker({ multiple: false, title: 'Seleccionar logotipo' });
  ...
  function renderBgList() {
    ...
    if (!dirState.background_images.length) {
      list.innerHTML = `<div style="font-size:12px;color:var(--text-muted);font-style:italic;padding:4px 0">${'No se han seleccionado imágenes de fondo'}</div>`;
      return;
    }
    list.innerHTML = `<div style="display:flex;gap:8px;flex-wrap:wrap">${
      dirState.background_images.map((u, i) => `
        <div style="position:relative;width:90px;height:68px;border-radius:4px;overflow:hidden;background:var(--bg-input);border:1px solid var(--border)">
          ...
          <button type="button" data-bg-remove="${i}" title="${'Quitar'}" style="position:absolute;top:3px;right:3px;width:22px;height:22px;border-radius:50%;border:0;background:rgba(0,0,0,0.75);color:#fff;cursor:pointer;font-size:14px;line-height:1;padding:0">&#215;</button>
        </div>
      `).join('')
    }</div>`;
  ...
  async function pickBgImages() {
    const urls = await openContentPicker({ multiple: true, title: 'Seleccionar imágenes de fondo' });
    if (urls && urls.length) {
  ...
  document.getElementById('saveWidgetBtn').onclick = async () => {
    ...
    try {
      ...
      showToast('Widget guardado', 'success');
      loadWidgets();
  ...
  document.getElementById('previewWidgetBtn').onclick = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Falló la previsualización');
  ...
  async function loadWidgets() {
    ...
    if (!widgets.length) {
      grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><h3>${'Aún no hay widgets'}</h3><p>${'Crea un widget para agregar contenido dinámico a tus diseños.'}</p></div>`;
      return;
    }
      ...
      return `
        <div class="content-item">
          ...
          <div class="content-item-actions">
            ...
            <button class="btn btn-danger btn-sm" data-delete-widget="${escAttr(w.id)}">${'Eliminar'}</button>
          </div>
        </div>
      `;
    }).join('');
    ...
    grid.onclick = async (e) => {
      ...
      if (deleteBtn) {
        ...
        try {
          ...
          showToast('Widget eliminado', 'success');
          loadWidgets();
        } catch (err) { showToast(err.message, 'error'); }
      }
    };
