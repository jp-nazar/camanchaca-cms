...
export function render(container) {
  container.innerHTML = `
    <div class="page-header">
      <div>
        <h1>${'Biblioteca de contenido'} <span class="help-tip" data-tip="${'Sube videos e imágenes aquí. Selecciona varios archivos para subir en lote. Usa URL remota para transmitir desde fuentes externas. Haz clic en una miniatura para previsualizar.'}">?</span></h1>
        <div class="subtitle">${'Sube y administra tus archivos multimedia'}</div>
      </div>
    </div>
    ...
    <div class="content-toolbar" style="display:flex;gap:16px;margin-bottom:24px">
      <div class="upload-area" id="uploadArea" style="flex:1;margin-bottom:0">
        ...
        <p class="upload-hint">${'Soporta MP4, WebM, AVI, MKV, JPEG, PNG, GIF, WebP'}</p>
        <input type="file" id="fileInput" style="display:none" multiple accept="video/*,image/*">
        <div class="upload-progress" id="uploadProgress" style="display:none">
          <div class="upload-progress-bar">
            <div class="upload-progress-fill" id="uploadProgressFill" style="width:0%"></div>
          </div>
          <p style="font-size:12px;color:var(--text-secondary);margin-top:6px" id="uploadProgressText">${'Subiendo...'}</p>
        </div>
      </div>
      <div style="width:320px;background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px;display:flex;flex-direction:column;gap:12px">
        ...
        <p style="font-size:12px;color:var(--text-muted)">${'Transmite directamente desde una URL. Ahorra ancho de banda local.'}</p>
        <input type="text" id="remoteUrlInput" class="input" placeholder="${'https://ejemplo.com/video.mp4'}">
        <input type="text" id="remoteNameInput" class="input" placeholder="${'Nombre para mostrar (opcional)'}">
    ...
    <div style="display:flex;gap:12px;margin-bottom:12px;align-items:center;flex-wrap:wrap">
      <input type="text" id="contentSearch" class="input" placeholder="${'Buscar contenido...'}" style="max-width:250px;width:100%">
      <button class="btn btn-secondary btn-sm" id="newFolderBtn">${'+ Nueva carpeta'}</button>
    </div>
    ...
    <div class="content-grid" id="contentGrid">
      <div class="empty-state" style="grid-column:1/-1"><h3>${'Cargando...'}</h3></div>
    </div>
  `;
...
async function loadContent() {
  ...
  try {
    ...
    breadcrumb.innerHTML = `
      <a href="#" data-folder-nav="" style="color:var(--text-secondary);text-decoration:none">${'Todo el contenido'}</a>
      ${path.map(f => `
        <span style="color:var(--text-muted)">/</span>
        <a href="#" data-folder-nav="${f.id}" style="color:var(--text-primary);text-decoration:none">${esc(f.name)}</a>
      `).join('')}
      ${state.currentFolderId ? `
        <button class="btn btn-secondary btn-sm" id="renameFolderBtn" style="margin-left:auto">${'Renombrar'}</button>
        <button class="btn btn-danger btn-sm" id="deleteFolderBtn">${'Eliminar carpeta'}</button>
      ` : ''}
    `;
    ...
    if (deleteBtn) deleteBtn.onclick = async () => {
      if (!confirm('¿Eliminar esta carpeta? El contenido volverá al nivel raíz. Las subcarpetas también se eliminarán.')) return;
      try {
    ...
    if (!content.length) {
      grid.innerHTML = subfolders.length ? '' : `
        <div class="empty-state" style="grid-column:1/-1">
          ...
          <h3>${state.currentFolderId ? 'Esta carpeta está vacía' : 'Aún no hay contenido'}</h3>
          <p>${state.currentFolderId ? 'Arrastra contenido aquí o usa la acción Mover.' : 'Sube videos e imágenes para comenzar.'}</p>
        </div>
      `;
      return;
    }
    ...
    grid.innerHTML = content.map(c => `
      <div class="content-item" draggable="true" data-content-id="${c.id}" data-folder="${c.folder || ''}">
        <div class="content-item-preview">
          ${c.remote_url
            ? `<div class="video-icon" style="flex-direction:column;gap:4px">
                ...
                <span style="font-size:10px;color:var(--text-muted)">${'Remota'}</span>
              </div>`
            : c.thumbnail_path
        ...
        <div class="content-item-actions">
          <button class="btn btn-secondary btn-sm" data-edit-content="${c.id}" title="${'Editar'}">
          ...
          <button class="btn btn-danger btn-sm" data-delete-content="${c.id}" title="${'Eliminar'}">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    grid.onclick = async (e) => {
      ...
      setTimeout(() => {
        if (btn.dataset.confirming === 'true') {
          btn.dataset.confirming = 'false';
          btn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg> ${'Eliminar'}`;
    ...
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><h3>${'Error al cargar el contenido'}</h3><p>${esc(err.message)}</p></div>`;
  }
}
    ...
    <div class="modal" style="max-width:500px;width:95vw">
      ...
      <div class="modal-body">
        <div class="form-group">
          <label>${'Nombre del archivo / Mostrar como'}</label>
        ...
        <div class="form-group">
          ...
          <select id="editMimeType" class="input" style="background:var(--bg-input)">
            ...
            <option value="image/jpeg" ${contentItem.mime_type === 'image/jpeg' ? 'selected' : ''}>${'Imagen (JPEG)'}</option>
            <option value="image/png" ${contentItem.mime_type === 'image/png' ? 'selected' : ''}>${'Imagen (PNG)'}</option>
            <option value="image/gif" ${contentItem.mime_type === 'image/gif' ? 'selected' : ''}>${'Imagen (GIF)'}</option>
            <option value="image/webp" ${contentItem.mime_type === 'image/webp' ? 'selected' : ''}>${'Imagen (WebP)'}</option>
          </select>
        </div>
        <div class="form-group">
          <label>${'Carpeta'}</label>
        ...
        <div class="form-group">
          ...
          <p style="font-size:11px;color:var(--text-muted);margin-top:4px">${'Déjalo vacío para mantener el archivo actual'}</p>
        </div>
        ` : ''}
      </div>
