...
export function render(container, deviceId, opts = {}) {
  ...
  playbackHandler = (data) => {
    ...
    if (el && data.current_content_id) {
      el.textContent = 'Reproduciendo: ' + (data.current_content_id);
    }
  };
...
async function loadDevice(deviceId, activeTab = null) {
  ...
  try {
    ...
    contentEl.innerHTML = `
      <div class="device-header">
        <div class="device-header-left">
          ...
          ${device.owner_name || device.owner_email ? `<span style="font-size:12px;color:var(--text-muted)">${'Propietario: ' + (device.owner_name || device.owner_email)}</span>` : ''}
        </div>
        ...
        <div class="screenshot-container">
          ${device.screenshot
            ...
            : `<div class="no-screenshot" id="currentScreenshot">
                ...
                <span>${'No hay captura disponible. Haz clic en "Captura" para tomar una.'}</span>
              </div>`
          }
        </div>
        ` : ''}
      ...
      <div class="tab-content" id="tab-playlist">
        ...
        <div id="deviceDraftBanner" style="background:#78350f;border:1px solid #92400e;border-radius:var(--radius);padding:14px 20px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;gap:16px">
          <div style="display:flex;align-items:center;gap:10px;color:#fbbf24">
            ...
            <div>
              <div style="font-weight:600;font-size:14px">${'Cambios sin publicar'}</div>
              <div style="font-size:12px;color:#fcd34d;opacity:0.85">${device.playlist_has_published ? 'Los dispositivos siguen mostrando la última versión publicada.' : 'Esta lista nunca se ha publicado. Los dispositivos no mostrarán nada hasta que la publiques.'}</div>
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-shrink:0">
            ${device.playlist_has_published ? `<button class="btn btn-secondary btn-sm" id="deviceDiscardDraftBtn" style="color:#fbbf24;border-color:#92400e">${'Descartar'}</button>` : ''}
            <button class="btn btn-sm" id="devicePublishBtn" style="background:#f59e0b;color:#000;font-weight:600;border:none">${'Publicar'}</button>
          </div>
        </div>
        ` : ''}
        <!-- Layout selector -->
        <div style="background:var(--bg-secondary);border:1px solid var(--border);border-radius:var(--radius);padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;gap:12px">
          ...
          <div style="flex:1">
            <div style="font-size:12px;color:var(--text-muted);margin-bottom:4px">${'Diseño de pantalla'}</div>
            <select id="deviceLayoutSelect" class="input" style="background:var(--bg-input);padding:4px 8px;font-size:13px">
              <option value="">${'Pantalla completa (predeterminado)'}</option>
            </select>
          </div>
          <button class="btn btn-secondary btn-sm" id="applyLayoutBtn">${'Aplicar'}</button>
        </div>

        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
          <div style="display:flex;align-items:center;gap:12px">
            <h3 style="font-size:16px">${'Lista'}</h3>
            <select class="input" id="playlistPicker" style="font-size:12px;padding:4px 8px;width:200px">
              <option value="">${'Sin lista'}</option>
            </select>
          </div>
          <div style="display:flex;gap:6px">
            <button class="btn btn-secondary btn-sm" id="copyPlaylistBtn">${'Copiar a...'}</button>
      ...
      <div class="tab-content" id="tab-info">
        <div class="info-grid">
          ...
          <div class="info-card">
            <div class="info-card-label">${'Dirección IP'}</div>
          ...
          <div class="info-card">
            <div class="info-card-label">${'Almacenamiento'}</div>
            <div class="info-card-value small" id="telStorage">${latestTelemetry.storage_free_mb ? (formatBytes(latestTelemetry.storage_free_mb)) + ' libres' : '--'}</div>
          ...
          ` : `
          <div class="info-card">
            <div class="info-card-label">${'Tipo de reproductor'}</div>
            <div class="info-card-value small">${'Reproductor web'}</div>
          </div>
          `}
          ${device.android_version && !device.android_version.startsWith('Web/') ? `
          <div class="info-card">
          ...
          <div class="info-card">
            ...
            <div class="info-card-value small" id="telRam">${latestTelemetry.ram_free_mb ? (formatBytes(latestTelemetry.ram_free_mb)) + ' libres' : '--'}</div>
          </div>
          <div class="info-card">
        ...
        <div style="margin-top:20px">
          <h4 style="font-size:13px;margin-bottom:8px">${'Línea de tiempo (últimas 24 horas)'}</h4>
          ...
          <div style="display:flex;justify-content:space-between;margin-top:4px">
            <span style="font-size:10px;color:var(--text-muted)">${'hace 24h'}</span>
            <span style="font-size:10px;color:var(--text-muted)">${'Ahora'}</span>
          </div>
          <div style="display:flex;gap:12px;margin-top:8px;font-size:11px;color:var(--text-muted)">
            <span><span style="display:inline-block;width:10px;height:10px;background:var(--success);border-radius:2px;vertical-align:-1px"></span> ${'En línea'}</span>
            <span><span style="display:inline-block;width:10px;height:10px;background:var(--danger);border-radius:2px;vertical-align:-1px"></span> ${'Desconectado'}</span>
            <span><span style="display:inline-block;width:10px;height:10px;background:var(--bg-primary);border:1px solid var(--border);border-radius:2px;vertical-align:-1px"></span> ${'Sin datos'}</span>
        ...
        <div style="margin-top:20px">
          <div style="display:flex;gap:12px;margin-bottom:12px">
            <div class="form-group" style="flex:1;margin:0">
              <label>${'Orientación / Rotación'}</label>
              <select id="deviceOrientation" class="input" style="background:var(--bg-input)">
                <option value="landscape" ${'landscape' === (device.orientation || 'landscape') ? 'selected' : ''}>${'Horizontal (0°)'}</option>
                <option value="portrait" ${'portrait' === device.orientation ? 'selected' : ''}>${'Vertical (90° SR)'}</option>
                <option value="landscape-flipped" ${'landscape-flipped' === device.orientation ? 'selected' : ''}>${'Horizontal invertido (180°)'}</option>
                <option value="portrait-flipped" ${'portrait-flipped' === device.orientation ? 'selected' : ''}>${'Vertical invertido (270° SR)'}</option>
              </select>
            </div>
            <div class="form-group" style="flex:1;margin:0">
              <label>${'Contenido predeterminado'}</label>
              <select id="deviceDefaultContent" class="input" style="background:var(--bg-input)">
                <option value="">${'Ninguno (mostrar "Esperando...")'}</option>
              </select>
            </div>
          </div>
          <div class="form-group">
            <label>${'Notas'}</label>
            <textarea id="deviceNotes" class="input" rows="3" placeholder="${'Ubicación, detalles de instalación, etc.'}" style="resize:vertical">${esc(device.notes || '')}</textarea>
          </div>
          <button class="btn btn-secondary btn-sm" id="saveNotesBtn">${'Guardar configuración'}</button>
        </div>
          ...
          <div class="remote-screen" id="remoteScreen">
            ...
            <div class="no-screenshot" id="remoteOverlay" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center">
              <div style="text-align:center">
                ...
                <p style="color:var(--text-secondary)">${'Haz clic en "Iniciar control remoto" para comenzar'}</p>
              </div>
            </div>
          </div>
          <div class="remote-controls">
            <button class="btn btn-primary" id="startRemoteBtn">${'Iniciar control remoto'}</button>
            <button class="btn btn-secondary" id="stopRemoteBtn" style="display:none">${'Detener control remoto'}</button>
            ...
            <div id="systemViewControls" style="opacity:0.4;pointer-events:none">
              <button class="btn btn-secondary btn-sm" onclick="window._sendKey('KEYCODE_HOME')">${'Inicio'}</button>
              <button class="btn btn-secondary btn-sm" onclick="window._sendKey('KEYCODE_BACK')">${'Atrás'}</button>
              <button class="btn btn-secondary btn-sm" onclick="window._sendKey('KEYCODE_APP_SWITCH')">${'Recientes'}</button>
              <button class="btn btn-danger btn-sm" onclick="window._sendKey('KEYCODE_POWER')">${'Encendido'}</button>
              <hr style="border-color:var(--border);margin:8px 0">
              ...
              <hr style="border-color:var(--border);margin:8px 0">
              <button class="btn btn-secondary btn-sm" onclick="window._sendCmd('settings')">${'Ajustes'}</button>
              <hr style="border-color:var(--border);margin:8px 0">
              <div style="display:flex;gap:4px">
                <button class="btn btn-secondary btn-sm" style="flex:1" onclick="window._sendCmd('screen_off')">${'Pant. off'}</button>
                <button class="btn btn-secondary btn-sm" style="flex:1" onclick="window._sendCmd('screen_on')">${'Pant. on'}</button>
              </div>
            </div>
            <button class="btn btn-primary btn-sm" id="enableSystemCaptureBtn" onclick="window._enableSystemView()" title="${'Solicita al usuario del dispositivo permitir captura de pantalla completa - habilita ver pantalla de inicio, ajustes y otras apps'}" style="margin-top:8px">
              ${'Habilitar vista de sistema'}
            </button>
            <span id="systemViewHint" style="font-size:10px;color:var(--text-muted);line-height:1.2;display:block;margin-top:4px">${'Requiere aprobación única en el dispositivo'}</span>
          </div>
        </div>
      </div>
    ` : ''}
    ...
    window._enableSystemView = () => {
      ...
      if (btn) { btn.textContent = 'Esperando aprobación del dispositivo...'; btn.disabled = true; }
      ...
      setTimeout(() => {
        ...
        if (btn) { btn.textContent = 'Vista de sistema habilitada'; btn.style.background = 'var(--success)'; }
        if (hint) hint.textContent = 'Navegación y controles del sistema desbloqueados';
      }, 5000);
    };
    ...
    contentEl.innerHTML = `<div class="empty-state"><h3>${'Error al cargar el dispositivo'}</h3><p>${esc(err.message)}</p></div>`;
  }
}

function renderPlaylist(assignments) {
  if (!assignments.length) {
    return `<div class="empty-state"><h3>${'Sin contenido asignado'}</h3><p>${'Agrega contenido de tu biblioteca a la lista de esta pantalla.'}</p></div>`;
  }
  return assignments.map((a, i) => `
    <div class="playlist-item" data-assignment-id="${a.id}" draggable="true" data-sort="${i}">
      ...
      <div class="playlist-item-actions" style="display:flex;align-items:center;gap:4px">
        ...
        <button class="btn-icon mute-toggle" data-mute-assignment="${a.id}" data-muted="${a.muted ? '1' : '0'}" title="${a.muted ? 'Activar audio' : 'Silenciar'}" style="color:${a.muted ? 'var(--danger)' : 'var(--text-muted)'}">
          ${a.muted
        ...
        <button class="btn-icon" title="${'Eliminar'}" data-remove-assignment="${a.id}">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
async function setupActions(device) {
  ...
  document.getElementById('renameBtn')?.addEventListener('click', async () => {
    const name = prompt('Ingresa el nuevo nombre:', device.name);
    ...
    api.getPlaylists().then(playlists => {
      playlists.forEach(p => {
        ...
        opt.textContent = p.is_auto_generated
          ? (p.name) + ' (auto) — ' + (p.item_count) + ' elementos'
          : (p.name) + ' — ' + (p.item_count) + ' elementos';
  ...
  deleteBtn?.addEventListener('click', async () => {
    ...
    deleteBtn.textContent = 'Haz clic de nuevo para confirmar';
  ...
  rebootBtn?.addEventListener('click', () => {
    ...
    rebootBtn.textContent = 'Haz clic de nuevo para confirmar';
  ...
  shutdownBtn?.addEventListener('click', () => {
    if (shutdownConfirming) {
      ...
      shutdownBtn.textContent = 'Apagar';
      return;
    }
    shutdownConfirming = true;
    shutdownBtn.textContent = 'Haz clic de nuevo para confirmar';
  ...
  try {
    ...
    if (select) {
      ...
      layouts.filter(l => l.is_template).forEach(l => {
        const opt = document.createElement('option');
        opt.value = l.id;
        opt.textContent = '[Plantilla] ' + (l.name) + ' (' + (l.zones?.length || 0) + ' zonas)';
        if (device.layout_id === l.id) opt.selected = true;
        select.appendChild(opt);
      });
    }
      ...
      modal.innerHTML = `
        <div class="modal" style="max-width:650px;width:95vw">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <select id="assignZone" class="input" style="background:var(--bg-input)">
                <option value="">${'Predeterminada (pantalla completa)'}</option>
          ...
          <div class="modal-footer">
            ...
            <button class="btn btn-primary" id="confirmAssign">${'Agregar selección'}</button>
          </div>
        </div>
      `;
...
function renderUptimeTimeline(uptimeData, statusLog = []) {
  ...
  if (percentEl) {
    percentEl.textContent = knownSlots > 0
      ? (uptimePct) + '% activo (' + (knownSlots * 15) + 'min registrados)'
      : (uptimePct) + '% activo (sin datos)';
  }
  ...
  timeline.innerHTML = slotStatus.map((status, i) => {
    ...
    const statusLabel = status === 'unknown' ? 'Sin datos' : status === 'online' ? 'En línea' : 'Desconectado';
...
function updateTelemetryDisplay(telemetry) {
  ...
  if (telemetry.storage_free_mb) update('telStorage', (formatBytes(telemetry.storage_free_mb)) + ' libres');
  ...
  if (telemetry.ram_free_mb) update('telRam', (formatBytes(telemetry.ram_free_mb)) + ' libres');
